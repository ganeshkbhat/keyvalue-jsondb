# keyvalue-jsondb
*fast, secure, private, and `memory leak resistant` `redis-like` `key-value`` `json based` data store or database that supports http, https, ws, wss, and command shell (without authentication) and is extendible with expressjs middlewares*
